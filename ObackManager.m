...
- (void)handlePan:(UIPanGestureRecognizer *)pan {
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateFailed:          // ← 纵向为主等导致手势失败，必须清理胶囊+重置状态
...
- (void)beginTransition:(UIPanGestureRecognizer *)pan {
    ...
    if (nav && nav.viewControllers.count > 1) {
        ...
        // 仍用手势驱动交互转场；注册 ObackTransitioningDelegate 并保留 App 原有 modal 动画为 original。
        OBLog(@"beginTransition: dismiss modal (方案B: 手势驱动视差, 只移 sheet 不碰 presenting)");
        self.interactive = [[ObackInteractiveTransition alloc] initWithEdge:self.currentEdge params:p];
        self.interactive.parallaxToView = NO;     // 方案B 关键：只动 sheet
        self.currentParallaxToView = NO;          // 供 delegate 创建 ObackAnimator 时读取
        self.interacting = YES;
        _indicatorAnchor = loc;
        _indicatorStartX = loc.x;
        [self showIndicatorWithEdge:self.currentEdge atPoint:loc inWindow:win];  // 有跟手视差，胶囊有意义
        // 注入转场转发器（仅本次 dismiss），保留 App 原有 modal 动画(original)。
        // 复用已存在的 td（同一仍present的弹窗再次内滑），避免释放仍被 VC.transitioningDelegate 引用的旧 td 导致野指针。
        ...
        // VC 自身 retain 转发器（UIKit 的 transitioningDelegate 是 assign，不会 retain），
        // 否则 cancel 后弹窗仍 present、manager 若释放 td 会留下野指针。
...
- (void)endTransition:(UIPanGestureRecognizer *)pan {
    ...
    OBLog(@"endTransition (percent=%.2f vel=%.0f projected=%.2f commit=%d)",
          _currentPercent, vel, projected, commit);
...
- (void)abortTransition:(UIPanGestureRecognizer *)pan {
    ...
    ObackParams *p = [ObackPreferences params];
    if (_indicator) [self dismissIndicatorCommitted:NO params:p window:win];
    // 若交互转场已启动则 cancel（未启动时 interactive=nil，调用 cancel 安全无操作）
    if (self.interactive) [self.interactive cancel];
    self.interacting = NO;
    self.interactive = nil;
    _currentPercent = 0;
}
